...
Audio activation and suspension reconcile with the latest user request, since [Web Audio context transitions are asynchronous](https://www.w3.org/TR/webaudio/#dom-audiocontext-resume). Failures belonging to an earlier request cannot stop the current session. Worker cache version `nestorium-v8` includes quieter-input detection, microphone-level feedback and capture startup recovery. Online loading can continue when cache storage is unavailable.
