...
Audio activation and suspension reconcile with the latest user request, since [Web Audio context transitions are asynchronous](https://www.w3.org/TR/webaudio/#dom-audiocontext-resume). Failures belonging to an earlier request cannot stop the current session. Worker cache version `nestorium-v10` includes frequency readouts above their wheels, an always-visible centre waveform and anchored wheels when capture details expand. Online loading can continue when cache storage is unavailable.
