...
Further suites cover note naming for two-decimal frequencies, synchronous presets, fine-tune and pitch-bend offsets, the wheel pointer at 4200 Hz, the fade-based voice release, Play Overtones, journeys on a hidden page, the 0 Hz guard, instrument-like capture signals (beating bowls, louder partials, hum, rumble) and the Intervals table under Piano Reference (cell names, octave selector, note-name headers, both wheels set and unmuted).

Stop, Reset and every switch between activities fade the voices out over 30 ms before stopping them, and lesson, theory and overtones-demo fade-outs are anchored ramps rather than steps. Scripted journeys keep running while the page is hidden or the phone screen is off: the scheduler drives them from a 250 ms timer whenever animation frames stop, and the oscillators follow the wheels. To check, start a journey, hide the tab or lock the phone for a minute and confirm the readouts and audio have advanced on return. A wheel at 0 Hz is silent rather than a held DC level. On iOS, a context can only start inside a gesture that ends (touch release, tap, key), so a wheel drag now becomes audible when the finger lifts: any such gesture resumes a suspended context that the transport wants running, and a tap never undoes Pause. Check on an iPad: rotate a wheel pointer from a fresh page and confirm sound starts on release without pressing Play.

Piano Reference has an Intervals button that expands a 12 × 12 table: rows are the lower note, columns the upper note, both in the selected octave (4 by default). When the column note is at or below the row note the upper note wraps into the next octave, so C to C is an octave and C# to C a major seventh. Tapping a cell sets the left wheel to the lower note and the right wheel to the upper note, clears tuning offsets, unmutes both wheels and starts playback; the keyboard scrolls to show both keys and the cell stays highlighted. Changing the octave replays the highlighted pair. Check on a phone that the row labels stay pinned while the table scrolls sideways, and on an iPad that a tap plays at once (a tap is a completed gesture, so no extra Play press is needed).
